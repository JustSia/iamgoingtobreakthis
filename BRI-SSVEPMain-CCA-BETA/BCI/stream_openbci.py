...
class StreamOpenBCI(threading.Thread):

    def __init__(self, stream_name, channels=8, srate=500, time=5):
    ...
    def get_stream(self):
        st = resolve_stream('type', 'EEG')
        # print("list:", list(st))
        for s in st:
            print(s.name())
        self.stream = resolve_stream('name', self.stream_name)
        print('EEG streams', self.stream)
    ...
    def shift_eeg(self, eeg_block):
        ...
        self.eeg_final[:5, :-n_steps] = self.eeg_final[:5, n_steps:]
        self.eeg_final[:5, -n_steps:] = eeg_block[:5]
...
if __name__ == '__main__':
    ...
    mindo = StreamOpenBCI(stream_name="openbci_eeg1", srate=500, channels=2)
