...
#socket = UDP.UdpComms(udpIP="127.0.0.1", portTX=8015, portRX=8016, enableRX=True, suppressWarnings=True)     # Local PC Testing
            ...
            if data.startswith('(') and data.endswith(')'):
                ...
                if len(coordinate_parts) == 15:
                    try:
                        ...
                        # Separate into DroneARCoord1, DroneARCoord2, DroneARCoord3, DroneARCoord4, DroneARCoord5
                        DroneARCoord1 = np.array(coordinate_list[:3])
                        DroneARCoord2 = np.array(coordinate_list[3:6])
                    ...
                    print("Data does not contain exactly 15 coordinate parts.")
            else:
                print("Data is not in the expected format (missing parentheses).")

            # print('Unity Leader Pos: ({}, {}, {})'.format(DroneARCoord1[0],DroneARCoord1[1],DroneARCoord1[2]))
            # print('Unity Left   Pos: ({}, {}, {})'.format(DroneARCoord2[0],DroneARCoord2[1],DroneARCoord2[2]))
            # print('Unity Right  Pos: ({}, {}, {})'.format(DroneARCoord3[0],DroneARCoord3[1],DroneARCoord3[2])) 
